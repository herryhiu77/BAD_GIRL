...
module.exports = async function (api, createLine) {
	// ———————————————————— LOAD DATA ———————————————————— //
	console.log("");
	global.utils.banner.section("DATABASE", {
		accent: "#86efac",
		subtitle: "Loading thread, user and dashboard data"
	});
	...
	const validThreadCount = (global.db.allThreadData || []).filter(
		t => t && t.threadID && String(t.threadID).length > 15
	).length;

	log.info(
		'DATABASE',
		getText('loadData', 'loadThreadDataSuccess', validThreadCount)
	);
	...
	if (api && global.GoatBot.config.database.autoSyncWhenStart == true) {
		console.log("");
		global.utils.banner.section("AUTO SYNC", {
			accent: "#c4b5fd",
			subtitle: "Refreshing thread data from Facebook"
		});
		...
		try {
			...
			for (const threadInfo of allThreadInfo) {
				if (
					threadInfo.isGroup &&
					!allThreadData.some(thread => thread.threadID === threadInfo.threadID)
				) {
					...
					const threadRefreshed = await threadsData.refreshInfo(
						threadInfo.threadID,
						threadInfo
					);
					...
					allThreadData.splice(
						allThreadData.findIndex(
							thread => thread.threadID === threadInfo.threadID
						),
						1
					);
			...
			const allThreadDataDontHaveBot = allThreadData.filter(
				thread => !allThreadInfo.some(
					thread1 => thread.threadID === thread1.threadID
				)
			);
